...
function changeTraveloElementUI() {
    ...
    tjq(".fileinput input[type=file]").on("change", function() {
        ...
        if (slashIndex == -1) {
            slashIndex = fileName.lastIndexOf("index.html");
        }
        if (slashIndex != -1) {
...
tjq(window).load(function() {
    ...
    if (enableChaser == 1 && tjq('#content').length > 0 && tjq('#main-menu ul.menu').length > 0) {
        var forchBottom;
        var chaser = tjq('#main-menush').clone().hide().appendTo(document.body).wrap("<div class='chaser hidden-mobile'><div class='container'></div></div>");
